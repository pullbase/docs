...
Failed webhook deliveries are retried 3 times with exponential backoff (1s, 2s, 4s). If all retries fail, the event is logged but not persisted for later retry.
