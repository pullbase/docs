...
Returns detailed drift information for a server, including which packages, services, or files have drifted from the desired state.
...
```json Drifted
{
  ...
  "drift_details": {
    "packages": [
      {"name": "nginx", "expected": "1.24.0", "actual": "1.22.0"}
    ],
    "files": [
      {"path": "/etc/nginx/nginx.conf", "status": "modified"}
    ]
  }
}
```
